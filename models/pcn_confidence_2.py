...
class PCN_CONFIDENCE_2(nn.Module):
    """
    ...
    def __init__(self, num_dense=4096, latent_dim=1024, grid_size=2):
        ...
        self.final_conv = nn.Sequential(
            ...
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
            nn.Conv1d(64, 4, 1)  #changed 3 to 4 for confidence
        )
    ...
    def forward(self, xyz):
        ...
        seed = seed.reshape(B, -1, self.num_dense).to('cuda:1')                                           # (B, 2, num_fine)
        ...
        return coarse.contiguous().to('cuda:1'), fine.contiguous().to('cuda:1'), confidence.squeeze().contiguous().to('cuda:1')
...
'''
    def forward(self, xyz):
        ...
        fine = fine[:,:3,:] + point_feat
        fine_t = fine.transpose(1, 2)
        confidence = confidence[:,3,:]
        confidence = confidence.transpose(1, 2)
        #confidence = self.confidence_conv(feat).transpose(1,2)                                              # (B, 4, num_fine), fine point cloud
        fine = torch.cat([fine_t, confidence], dim=2)
        #fine = fine[:,:3,:]
