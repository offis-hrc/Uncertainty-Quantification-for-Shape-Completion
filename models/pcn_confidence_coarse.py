...
class PCN_CONFIDENCE(nn.Module):
    ...
    def forward(self, xyz):
        ...
        # decoder
        coarse_conf = self.mlp(feature_global).reshape(-1, self.num_coarse, 4)
        coarse = coarse_conf[:,:,:3]#self.mlp(feature_global).reshape(-1, self.num_coarse, 3)                     # (B, num_coarse, 3), coarse point cloud
        ...
        return coarse_conf.contiguous(), fine.contiguous(), confidence.squeeze().contiguous()
