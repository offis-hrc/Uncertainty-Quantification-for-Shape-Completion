...
from visualization import plot_pcd_one_view_confidence_test
from metrics.metric import l1_cd, l1_cd_point, l2_cd, emd, emd_point, f_score
...
def test_single_category(category, model, params, save=True):
    ...
    test_dataset = ShapeNet('/home/pmegarajan/Projects/nbv/PCN-PyTorch/data/PCN', 'test_novel' if params.novel else 'test', category)
    ...
    with torch.no_grad():
        for p, c in test_dataloader:
            ...
            coarse_pred, c_, confidence = model(p)
            #coarse_pred, c_ = model(p)
            total_l1_cd += l1_cd( c_[:,:,:3], c).item()
            print('Predddddddddddddd',c_.size())
            c_pts = c_[:,:,:3]
            print('Predddddddddddddd22222',c_pts.size())
            l2_cd_batch, cd_point1, cd_point2 = l1_cd_point(c_, c)#.item()
            #total_l2_cd, cd_point1, cd_point2 = l1_cd_point(c_pts, c)#.item()

            #total_l2_cd += l2_cd(coarse_pred, c).item()
            total_l2_cd += l2_cd_batch
            for i in range(len(c)):
                
                input_pc = p[i].detach().cpu().numpy()
                output_pc = c_[i].detach().cpu().numpy()
                coarse_output = coarse_pred[i].detach().cpu().numpy()
                gt_pc = c[i].detach().cpu().numpy()
                total_f_score += 0#f_score(output_pc, gt_pc)
                print('CD values 1',cd_point1.size(), cd_point1[i, :30])
                print('CD values 2',cd_point2.size(), cd_point2[i, :30])
                print('Pred Point Cloud', np.shape(output_pc), output_pc[:30])
                if save:
                    print('saving')
                    plot_pcd_one_view_confidence_test(os.path.join(image_dir, '{:03d}.png'.format(index)), [input_pc, coarse_output, output_pc, gt_pc], ['Input', 'View 1', 'View 2', 'GT'], cd_point2[i].detach().cpu().numpy(), xlim=(-0.35, 0.35), ylim=(-0.35, 0.35), zlim=(-0.35, 0.35))
                    #plot_pcd_one_view_confidence_test(os.path.join(image_dir, '{:03d}_error.png'.format(index)), [input_pc, output_pc, gt_pc], ['Input', 'Output', 'GT'], cd_point1[i].detach().cpu().numpy(), xlim=(-0.35, 0.35), ylim=(-0.35, 0.35), zlim=(-0.35, 0.35))

                    #export_ply(os.path.join(output_dir, '{:03d}.ply'.format(index)), output_pc)
                    partial_array = p[i].cpu().detach().numpy().reshape((-1,3))
                    np.savetxt('partial_'+str(i)+'.txt', np.around(partial_array, 3))
                    dense_array = c_[i].cpu().detach().numpy().reshape((-1,4))
                    np.savetxt('pred_'+str(i)+'.txt', np.around(dense_array, 3))
                index += 1
...
def test_single_category_emd(category, model, params):
    ...
    with torch.no_grad():
        for p, c in test_dataloader:
            p = p.to(params.device)
            c = c.to(params.device)
            _, c_ , confidence = model(p)
            c_pts = c_[:,:,:3]
            dist = emd(c_pts, c).item()
            print(dist)
            total_emd += dist
            #total_emd = emd_point(c_[:,:,:3], c)#.item()
            #print('EMD values', total_emd[:])
