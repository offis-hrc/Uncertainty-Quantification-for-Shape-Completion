					...
					for (int k=0;k<batch;k+=4){
						{
							...
							float y2=buf[k*3+1];
							float z2=buf[k*3+2];
							float d=(x2 - (x1 + best_c*(x2-x1)))*(x2 - (x1 + best_c*(x2-x1)))+(y2 - (y1 + best_c*(y2-y1)))*(y2 - (y1 + best_c*(y2-y1)))+(z2 - (z1 + best_c*(z2-z1)))*(z2 - (z1 + best_c*(z2-z1))) + 0.003*(0-best_c)*(0-best_c);
							if (k==0 || d<best){
								best=d;
								best_x1 = x1;
								best_y1 = y1;
								...
								best_z2 = buf[k*3+2];
								//best_c  = buf[k*4+3];
								//best = (best_x2 - ((1-best_c)*best_x1 + best_c*best_x2))*(best_x2 - ((1-best_c)*best_x1 + best_c*best_x2)) +  (best_y2 - ((1-best_c)*best_y1 + best_c*best_y2))*(best_y2 - ((1-best_c)*best_y1 + best_c*best_y2)) + (best_z2 - ((1-best_c)*best_z1 + best_c*best_z2))*(best_z2 - ((1-best_c)*best_z1 + best_c*best_z2));
								best_i=k+k2;
							}
						}
						{
					...
					for (int k=0;k<batch;k+=4){
						{
							...
							float y2=buf[k*4+1];
							float z2=buf[k*4+2];
							float best_c  = buf[k*4+3];
							float d= (x1 - (x2 + best_c*(x1 -x2)))*(x1 - (x2 + best_c*(x1 -x2))) +(x1 - (x2 + best_c*(x1 -x2)))*(x1 - (x2 + best_c*(x1 -x2))) + (x1 - (x2 + best_c*(x1 -x2)))*(x1 - (x2 + best_c*(x1 -x2))) + 0.002*(best_c)*(best_c);
							if (k==0 || d<best){
								best=d;
								best_i=k+k2;
								best_x1 = x1;
