		...
		for (int k2=0;k2<m;k2+=batch){
			...
			for (int j=threadIdx.x+blockIdx.y*blockDim.x;j<n;j+=blockDim.x*gridDim.y){
				...
				if (k2==0 || result[(i*n+j)]>best){
					...
					best = (best_x2 - (best_x1 + best_c*(best_x2-best_x1)))*(best_x2 - (best_x1 + best_c*(best_x2-best_x1))) + (best_y2 - (best_y1 + best_c*(best_y2-best_y1)))*(best_y2 - (best_y1 + best_c*(best_y2-best_y1))) + (best_z2 - (best_z1 + best_c*(best_z2-best_z1)))*(best_z2 - (best_z1 + best_c*(best_z2-best_z1))) +  0.003*(0-best_c)*(0-best_c);
