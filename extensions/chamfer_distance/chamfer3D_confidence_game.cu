		...
		for (int k2=0;k2<m;k2+=batch){
			...
			for (int j=threadIdx.x+blockIdx.y*blockDim.x;j<n;j+=blockDim.x*gridDim.y){
				...
				if (k2==0 || result[(i*n+j)]>best){
                    ...
                    best = (best_x1 - (best_x2 + best_c*(best_x1-best_x2)))*(best_x1 - (best_x2 + best_c*(best_x1-best_x2))) + (best_y1 - (best_y2 + best_c*(best_y1-best_y2)))*(best_y1 - (best_y2 + best_c*(best_y1-best_y2))) + (best_z1 - (best_z2 + best_c*(best_z1-best_z2)))*(best_z1 - (best_z2 + best_c*(best_z1-best_z2))) + 0.02*(1-best_c)*(1-best_c);
...
__global__ void NmDistanceGradKernelNormal(int b,int n,const float * xyz1,int m,const float * xyz2,const float * grad_dist1,const int * idx1,float * grad_xyz1,float * grad_xyz2){
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		for (int j=threadIdx.x+blockIdx.y*blockDim.x;j<n;j+=blockDim.x*gridDim.y){
			...
			atomicAdd(&(grad_xyz2[(i*m+j2)*4+3]),((g)*(((c-1)*(x1*x1 + x2*x2 + y1*y1 + y2*y2 + z1*z1 +z2*z2 - 2*x1*x2 - 2*y1*y2 - 2*z1*z2)) - 0.04*(1-c))));
