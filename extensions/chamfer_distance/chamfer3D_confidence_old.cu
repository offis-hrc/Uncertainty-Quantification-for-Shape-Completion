...
__global__ void NmDistanceGradKernel(int b,int n,const float * xyz1,int m,const float * xyz2,const float * grad_dist1,const int * idx1,float * grad_xyz1,float * grad_xyz2){
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		for (int j=threadIdx.x+blockIdx.y*blockDim.x;j<n;j+=blockDim.x*gridDim.y){
			...
			atomicAdd(&(grad_xyz1[(i*n+j)*3+0]),g*c*c*(x1-x2));
			atomicAdd(&(grad_xyz1[(i*n+j)*3+1]),g*c*c*(y1-y2));
			atomicAdd(&(grad_xyz1[(i*n+j)*3+2]),g*c*c*(z1-z2));
			atomicAdd(&(grad_xyz2[(i*m+j2)*4+0]),-(g*c*c*(x1-x2)));
			atomicAdd(&(grad_xyz2[(i*m+j2)*4+1]),-(g*c*c*(y1-y2)));
			atomicAdd(&(grad_xyz2[(i*m+j2)*4+2]),-(g*c*c*(z1-z2)));
