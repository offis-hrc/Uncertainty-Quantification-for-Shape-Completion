	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		...
		for (int k=kbeg;k<kend;k++){
			...
			for (int j=threadIdx.x;j<n;j+=blockDim.x){
				...
				scalar_t x1=x2-xyz1[(i*n+j)*4+0];//x2-(c*xyz1[(i*n+j)*4+0] + (1-c)*x2);
				scalar_t y1=y2-xyz1[(i*n+j)*4+1];//y2-(c*xyz1[(i*n+j)*4+1] + (1-c)*y2);
				scalar_t z1=z2-xyz1[(i*n+j)*4+2];//z2-(c*xyz1[(i*n+j)*4+2] + (1-c)*z2);
	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		for (int l=threadIdx.x;l<n;l+=blockDim.x){
			...
			for (int k=0;k<m;k++){
				...
				dx+=(x1-x2)*d; //d*(x1-(c*x2+(1-c)*x1));
				dy+=(y1-y2)*d; //d*(y1-(c*y2+(1-c)*y1));
				dz+=(z1-z2)*d; //d*(z1-(c*z2+(1-c)*z1));
				dc+=0.;//d*(c-1)*((x1*x1 - 2*x1*x2 + y1*y1 -2*y1*y2 + z1*z1 - 2*z1*z2 + x2*x2 + y2*y2 + z2*z2) - 100*(0-c));
            ...
            grad1[i*n*4+l*4+3]=0.*dc*grad_cost[i];
		}
	}
}
