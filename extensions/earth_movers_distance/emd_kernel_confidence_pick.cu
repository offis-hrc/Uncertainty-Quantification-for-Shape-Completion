			...
			for (int l0=0;l0<m;l0+=Block){
				...
				if (k<n){
					for (int l=0;l<lend;l++){
						...
						subsum+=c*d*match[i*n*m+(l0+l)*n+k];
					}
				}
				__syncthreads();
			}
	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		...
		for (int k=kbeg;k<kend;k++){
			...
			for (int j=threadIdx.x;j<n;j+=blockDim.x){
				...
				scalar_t x1=x2-xyz1[(i*n+j)*4+0];
				scalar_t y1=y2-xyz1[(i*n+j)*4+1];
				scalar_t z1=z2-xyz1[(i*n+j)*4+2];
	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		for (int l=threadIdx.x;l<n;l+=blockDim.x){
			...
			for (int k=0;k<m;k++){
				...
				scalar_t d=match[i*n*m+k*n+l];
				dx+=d*(x1-x2);
				dy+=d*(y1-y2);
				dz+=d*(z1-z2);
                dc+=d*(x1-x2)*(x1-x2) + d*(y1-y2)*(y1-y2) + d*(z1-z2)*(z1-z2);
			}
