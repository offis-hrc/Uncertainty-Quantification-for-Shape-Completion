			...
			for (int l0=0;l0<m;l0+=Block){
				...
				if (k<n){
					for (int l=0;l<lend;l++){
                        ...
                        scalar_t d=(x2-(x1 + c*(x2-x1)))*(x2-(x1 + c*(x2-x1))) + (y2-(y1 + c*(y2-y1)))*(y2-(y1 + c*(y2-y1))) + (z2-(z1 + c*(z2-z1)))*(z2-(z1 + c*(z2-z1))) + 0.002*(0-c)*(0-c);
	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		for (int l=threadIdx.x;l<n;l+=blockDim.x){
			...
			for (int k=0;k<m;k++){
				...
				dc+=d*(c-1)*((x1*x1 - 2*x1*x2 + y1*y1 -2*y1*y2 + z1*z1 - 2*z1*z2 + x2*x2 + y2*y2 + z2*z2) - 0.004*(0-c));
			}
