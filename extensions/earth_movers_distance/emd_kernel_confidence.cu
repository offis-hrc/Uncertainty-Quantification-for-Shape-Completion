			...
			for (int l0=0;l0<m;l0+=blockDim.x){
				...
				for (int k0=0;k0<n;k0+=Block){
					...
					for (int k=threadIdx.x;k<kend;k+=blockDim.x){
						buf[k*4+0]=xyz1[i*n*4+k0*4+k*4+0];
						buf[k*4+1]=xyz1[i*n*4+k0*4+k*4+1];
						buf[k*4+2]=xyz1[i*n*4+k0*4+k*4+2];
			...
			for (int l0=0;l0<m;l0+=Block){
				...
				if (k<n){
					for (int l=0;l<lend;l++){
						...
						//scalar_t d=(x2-(c*x1+(1-c)*x2))*(x2-(c*x1+(1-c)*x2)) + (y2-(c*y1+(1-c)*y2))*(y2-(c*y1+(1-c)*y2)) + (z2-(c*z1+(1-c)*z2))*(z2-(c*z1+(1-c)*z2));
                        //scalar_t d=(x2-(x1 + c*(x2-x1)))*(x2-(x1 + c*(x2-x1))) + (y2-(y1 + c*(y2-y1)))*(y2-(y1 + c*(y2-y1))) + (z2-(z1 + c*(z2-z1)))*(z2-(z1 + c*(z2-z1))) + 100*(0-c)*(0-c);
						scalar_t d=(x2-(x1 + c*(x2-x1)))*(x2-(x1 + c*(x2-x1))) + (y2-(y1 + c*(y2-y1)))*(y2-(y1 + c*(y2-y1))) + (z2-(z1 + c*(z2-z1)))*(z2-(z1 + c*(z2-z1))) + 0.003*(0-c)*(0-c);
...
__global__ void matchcostgrad2(int b,int n,int m,const scalar_t * __restrict__ grad_cost,const scalar_t * __restrict__ xyz1,const scalar_t * __restrict__ xyz2,const scalar_t * __restrict__ match,scalar_t * __restrict__ grad2){
	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		...
		for (int k=kbeg;k<kend;k++){
			...
			for (int j=threadIdx.x;j<n;j+=blockDim.x){
                scalar_t c =xyz1[(i*n+j)*4+3];
				scalar_t x1=(c-1)*(c-1)*(x2-xyz1[(i*n+j)*4+0]);//x2-xyz1[(i*n+j)*4+0];//x2-(c*xyz1[(i*n+j)*4+0] + (1-c)*x2);
				scalar_t y1=(c-1)*(c-1)*(y2-xyz1[(i*n+j)*4+1]);//y2-xyz1[(i*n+j)*4+1];//y2-(c*xyz1[(i*n+j)*4+1] + (1-c)*y2);
				scalar_t z1=(c-1)*(c-1)*(z2-xyz1[(i*n+j)*4+2]);//z2-xyz1[(i*n+j)*4+2];//z2-(c*xyz1[(i*n+j)*4+2] + (1-c)*z2);
				scalar_t d=match[i*n*m+k*n+j]*2;
				subsumx+=x1*d;
				subsumy+=y1*d;
				subsumz+=z1*d;
                //subsumc+=x1*d+y1*d+z1*d;
			}
			sum_grad[threadIdx.x*3+0]=subsumx;
			sum_grad[threadIdx.x*3+1]=subsumy;
			sum_grad[threadIdx.x*3+2]=subsumz;
            //sum_grad[threadIdx.x*3+3]=subsumc;
			for (int j=1;j<blockDim.x;j<<=1){
				__syncthreads();
				int j1=threadIdx.x;
				...
				if ((j1&j)==0 && j2<blockDim.x){
                    ...
                    //sum_grad[j1*3+3]+=sum_grad[j2*3+3];
				}
			}
			if (threadIdx.x==0){
                ...
                //grad2[(i*m+k)*3+3]=sum_grad[3]*grad_cost[i];
			}
			__syncthreads();
		}
	}
...
__global__ void matchcostgrad1(int b,int n,int m,const scalar_t * __restrict__ grad_cost,const scalar_t * __restrict__ xyz1,const scalar_t * __restrict__ xyz2,const scalar_t * __restrict__ match,scalar_t * __restrict__ grad1){
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		for (int l=threadIdx.x;l<n;l+=blockDim.x){
			scalar_t x1=xyz1[i*n*4+l*4+0];
			scalar_t y1=xyz1[i*n*4+l*4+1];
			scalar_t z1=xyz1[i*n*4+l*4+2];
            scalar_t c =xyz1[i*n*4+l*4+3];            
			scalar_t dx=0,dy=0,dz=0, dc=0;
			for (int k=0;k<m;k++){
				scalar_t x2=xyz2[i*m*3+k*3+0];
				scalar_t y2=xyz2[i*m*3+k*3+1];
				scalar_t z2=xyz2[i*m*3+k*3+2];                
				scalar_t d=match[i*n*m+k*n+l]*2;
				dx+= -d*(c-1)*(c-1)*(x2-x1); //(x1-x2)*d;
				dy+= -d*(c-1)*(c-1)*(y2-y1);//(y1-y2)*d;
				dz+= -d*(c-1)*(c-1)*(z2-z1);//(z1-z2)*d;
				dc+= d*((c-1)*(x1*x1 - 2*x1*x2 + y1*y1 -2*y1*y2 + z1*z1 - 2*z1*z2 + x2*x2 + y2*y2 + z2*z2) - 0.006*(0-c));//0.;
				//dc+=d*((x1-(c*x2+(1-c)*x1)) + (y1-(c*y2+(1-c)*y1)) + (z1-(c*z2+(1-c)*z1)));
			}
			grad1[i*n*4+l*4+0]=dx*grad_cost[i];
			grad1[i*n*4+l*4+1]=dy*grad_cost[i];
			grad1[i*n*4+l*4+2]=dz*grad_cost[i];
            grad1[i*n*4+l*4+3]=1*dc*grad_cost[i];
		}
	}
}
...
std::vector<at::Tensor> MatchCostBackward(
  ...
  auto grad1 = at::zeros({b, n, 4}, xyz1.type());
