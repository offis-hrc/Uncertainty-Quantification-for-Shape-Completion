			...
			for (int l0=0;l0<m;l0+=Block){
				...
				if (k<n){
					for (int l=0;l<lend;l++){
						...
						scalar_t d=(x2-(x1 + c*(x2-x1)))*(x2-(x1 + c*(x2-x1))) + (y2-(y1 + c*(y2-y1)))*(y2-(y1 + c*(y2-y1))) + (z2-(z1 + c*(z2-z1)))*(z2-(z1 + c*(z2-z1))) + 50*(0-c)*(0-c);
	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		...
		for (int k=kbeg;k<kend;k++){
			...
			for (int j=threadIdx.x;j<n;j+=blockDim.x){
				...
				scalar_t x1=(c-1)*(c-1)*(x2-xyz1[(i*n+j)*4+0]);//x2-(c*xyz1[(i*n+j)*4+0] + (1-c)*x2);
				scalar_t y1=(c-1)*(c-1)*(y2-xyz1[(i*n+j)*4+1]);//y2-(c*xyz1[(i*n+j)*4+1] + (1-c)*y2);
				scalar_t z1=(c-1)*(c-1)*(z2-xyz1[(i*n+j)*4+2]);//z2-(c*xyz1[(i*n+j)*4+2] + (1-c)*z2);
	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		for (int l=threadIdx.x;l<n;l+=blockDim.x){
			...
			for (int k=0;k<m;k++){
				...
				dx+=-d*(c-1)*(c-1)*(x2-x1); //d*(x1-(c*x2+(1-c)*x1));
				dy+=-d*(c-1)*(c-1)*(y2-y1); //d*(y1-(c*y2+(1-c)*y1));
				dz+=-d*(c-1)*(c-1)*(z2-z1); //d*(z1-(c*z2+(1-c)*z1));
                //dc+=d*((x1-(c*x2+(1-c)*x1)) + (y1-(c*y2+(1-c)*y1)) + (z1-(c*z2+(1-c)*z1)));
				dc+=d*(c-1)*((x1*x1 - 2*x1*x2 + y1*y1 -2*y1*y2 + z1*z1 - 2*z1*z2 + x2*x2 + y2*y2 + z2*z2) - 100*(0-c));
			}
			grad1[i*n*4+l*4+0]=dx*grad_cost[i];
			grad1[i*n*4+l*4+1]=dy*grad_cost[i];
			grad1[i*n*4+l*4+2]=dz*grad_cost[i];
            grad1[i*n*4+l*4+3]=dc*grad_cost[i];
		}
	}
}
